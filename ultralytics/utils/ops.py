...
def NMS(boxes,scorse,iou_thres,GIoU=True,CIoU=False,EIoU=False,SIoU=False):
    B = torch.argsort(scorse,dim=-1,descending=True)
    keep = []
    while B.numel() > 0:
        index = B[0]
        keep.append(index)
        if B.numel() == 1:
            break
        iou = bbox_iou(boxes[index,:],boxes[B[:],:],GIoU=GIoU,CIoU=CIoU,EIoU=EIoU,SIoU=SIoU)
